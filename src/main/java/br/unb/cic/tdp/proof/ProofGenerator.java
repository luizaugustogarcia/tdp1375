...
public class ProofGenerator {
    ...
    private static void generate(final String proofFolder, final String ehProofFolder) throws FileNotFoundException {
        ...
        OddCycles.generate().forEach(_case -> printCase.accept(_case, prop17Printer));
         ...
         */
        final var lemma24Unoriented = new PrintStream(new File(proofFolder + "\\lemma-24-unoriented.txt"));
        DesimplifyUnoriented.generate(ehProofFolder, lemma24Unoriented);

        final var lemma24Oriented = new PrintStream(new File(proofFolder + "\\lemma-24-oriented.txt"));
        DesimplifyOriented.generate(ehProofFolder, lemma24Oriented);

        // TODO standardize manipulation of spi, pi, rhos around Configuration
