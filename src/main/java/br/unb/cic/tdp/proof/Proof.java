...
public class Proof {
    ...
    public static void main(final String[] args) throws FileNotFoundException {
        generateProof(new PrintStream(new File("result.txt")));
    }
    ...
    private static void generateProof(final PrintStream printer) {
        final Consumer<Case> printCase = _case -> {
            printer.println(_case.getSigmaPiInverse() + ";"
                    + _case.getRhos().stream().map(Arrays::toString).collect(Collectors.joining("-")));
        };
        ...
        Cases3_2.generate().forEach(printCase);
