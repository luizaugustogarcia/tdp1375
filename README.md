...
where the first argument is the permutation to be sorted, e.g, `0,4,8,3,7,2,6,1,5,9,14,13,12,11,10`. Note that, before executing the algorithm itself, the program has to load into memory all cases generated in the case analysis. This demands a significant amount of memory (~20GB) and takes about 2 minutes to complete in an Intel i7™ vPro 8th Gen computer with 48GB of RAM. Therefore, we suggest, before running the algorithm, to increase the memory available to at least 20GB, by setting the environment variable MAVEN_OPTS="-Xmx20G".
